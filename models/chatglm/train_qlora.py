...
def parse_args():
    ...
    parser.add_argument('--train_args_json', type=str, default='chatGLM_6B_QLoRA.json', help='TrainingArguments的json文件')
    parser.add_argument('--model_name_or_path', type=str, default='THUDM/chatglm-6b', help='模型id或local path')
    parser.add_argument('--train_data_path', type=str, default='/home/yz/here/code/chatglm/data/yz-r100/moocdata13/moocdata13-train.json', help='训练数据路径')
    #parser.add_argument('--output_dir', type=str, default='/home/yz/here/code/chatglm/data/yz/MOOCQA13', help='训练数据路径')
    # parser.add_argument('--eval_data_path', type=str, default='data/sftdata-en-valid.jsonl', help='验证数据路径')
    # parser.add_argument('--eval_data_path', type=str, default='data/test_c.jsonl', help='验证数据路径')
...
def get_datset(data_path, tokenizer, global_args):
    ...
    data = Dataset.from_json(data_path).rename_columns({
        'input': 'instruction',
    })
    column_names = data.column_names
    dataset = data.map(lambda example: tokenize_func(example, tokenizer, global_args),
